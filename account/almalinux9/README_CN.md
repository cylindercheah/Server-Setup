...
1. 选择选项 `3`
2. 查看脚本显示的当前可选用户列表（UID>=1000，且非 `nobody`）
3. 输入用户名列表（空格分隔）
4. 输入VNC密码
...
3. 输入要修改角色的用户名
4. 选择目标角色：
  - `admin`：执行 `usermod -aG wheel 用户名`
  - `standard`：执行 `gpasswd -d 用户名 wheel`
5. 按提示确认角色变更

**说明：**

- `admin` 角色通过加入 `wheel` 组实现（可配合sudo策略执行管理操作）
- `standard` 角色不在 `wheel` 组
- 角色变更不会删除用户数据与VNC配置
...
### Q: `admin` 和 `standard` 有什么区别？
**A:** `admin` 用户会加入 `wheel` 组；`standard` 用户不在 `wheel`。这并不自动改变他人home目录权限，是否可访问仍由文件权限和sudo行为决定。

### Q: 可以后续修改用户角色吗？
**A:** 可以，使用选项5“修改现有用户角色（admin/standard）”。
...
- 自动设置用户密码
- 支持角色选择：`admin`（加入 `wheel`）或 `standard`
- 自动收紧home目录权限为 `700`
