...
# 批量创建用户并设置VNC密码脚本（随机密码版本）
# 使用方法：sudo ./create_with_vnc_random.sh
...
function generate_random_password() {
    ...
    # 排除容易混淆的字符：0、O、o、l、1、I
    local PASSWORD=$(tr -dc 'A-HJ-NP-Za-hj-np-z2-9' < /dev/urandom | head -c "$LENGTH")
    echo "$PASSWORD"
}
...
function get_user_range() {
    while true; do
        ...
        # 询问密码长度
        read -p "密码长度 (8-32，默认12): " CUSTOM_LENGTH
        if [ -n "$CUSTOM_LENGTH" ]; then
            if [[ $CUSTOM_LENGTH =~ ^[0-9]+$ ]] && [ "$CUSTOM_LENGTH" -ge 8 ] && [ "$CUSTOM_LENGTH" -le 32 ]; then
                PASSWORD_LENGTH=$CUSTOM_LENGTH
            else
                echo "警告：密码长度无效，使用默认值: 12"
                PASSWORD_LENGTH=12
            fi
        fi
        ...
        echo "密码模式: 随机生成（每个用户独立）"
        echo "密码长度: $PASSWORD_LENGTH 位"
        echo "默认Shell: /bin/zsh"
        echo "密码保存: $PASSWORD_FILE"
        echo "======================================"
...
if [ ${#CREATED_USERS[@]} -gt 0 ]; then
    ...
    for i in "${!CREATED_USERS[@]}"; do
        USER_INFO="${CREATED_USERS[$i]}"
        PASS_INFO="${USER_PASSWORDS[$i]}"
        USERNAME="${USER_INFO%%:*}"
        PORT="${USER_INFO##*:}"
        PASSWORD="${PASS_INFO##*:}"
        VNC_PORT=$((5900 + PORT))
...
echo "======================================"
echo "重要提示："
echo "1. 每个用户都有独立的随机密码（已列在上方表格中）"
echo "2. 所有密码已保存到文件:"
echo "   - 文本格式: $PASSWORD_FILE"
echo "   - CSV格式:  $CSV_FILE"
echo "3. 请妥善保管密码文件，建议备份到安全位置"
echo "4. 密码文件权限已设置为 600 (仅root可读写)"
echo "5. 所有用户的默认shell为: /bin/zsh"
echo "6. VNC配置文件位于各用户的 ~/.vnc/passwd"
echo "7. VNC启动脚本位于各用户的 ~/.vnc/xstartup"
echo "8. VNC端口配置文件: /etc/tigervnc/vncserver.users"
echo "9. 端口号与用户编号一致 (user001 -> 端口1 -> TCP 5901)"
echo "10. VNC系统服务已自动启用并启动 (vncserver@:<端口>.service)"
echo "11. 防火墙端口已自动开放 (如果使用firewalld)"
echo ""
echo "查看密码文件："
echo "  - 文本格式: cat $PASSWORD_FILE"
echo "  - CSV格式:  cat $CSV_FILE"
echo "  - 下载CSV:  scp root@<服务器IP>:$CSV_FILE ."
echo ""
echo "RealVNC Viewer 连接设置："
echo "  - 服务器地址: <IP>:<端口> (例如 192.168.1.100:1)"
echo "  - 加密方式: Prefer off (TigerVNC不支持RealVNC加密)"
echo "  - 认证密码: 使用对应用户的VNC密码"
echo ""
echo "常用命令："
...
echo ""
echo "如需删除用户，请使用配套的删除脚本："
echo "  sudo ./delete_vnc_user.sh"
echo ""
echo "前置要求："
